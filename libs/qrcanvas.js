(function (global, factory) {
  ...
  function () {
    function QRCanvas(options) {
      ...
      _defineProperty(this, "isDark", function (i, j) {
        var _this$qrdata = _this.qrdata,
            count = _this$qrdata.count,
            qr = _this$qrdata.qr;
        return i >= 0 && i < count && j >= 0 && j < count && _this.shouldTransclude(i * count + j) && qr.isDark(i, j);
      });
    ...
    _createClass(QRCanvas, [{
      ...
      value: function setData(userOptions) {
        var options = _objectSpread({}, defaultOptions(), userOptions, {
          effect: _objectSpread({}, userOptions.effect)
        });

        if (userOptions.logo && (userOptions.logo.image || userOptions.logo.text)) {
          var logo = _objectSpread({}, defaultLogoOptions(), userOptions.logo);
      ...
      value: function makeQR() {
        var _this$options = this.options,
            ...
            correctLevel = _this$options.correctLevel,
            data = _this$options.data,
            cellSize = _this$options.cellSize;
      ...
      value: function makeBackground() {
        var _this$options2 = this.options,
            noAlpha = _this$options2.noAlpha,
      ...
      value: function makeForeground() {
        ...
        var _this$options3 = this.options,
            foreground = _this$options3.foreground,
            effect = _this$options3.effect;
        var _this$qrdata2 = this.qrdata,
            cellSize = _this$qrdata2.cellSize,
            size = _this$qrdata2.size;
        ...
        this.cache.foreground = (effectInfo.scenes || [{}]).map(function (scene) {
          var mask = QRCanvas.getCanvas({
            width: size
          });
          effectInfo.draw(_objectSpread({
            effect: effect,
            canvas: mask,
          ...
          var canvas = QRCanvas.drawCanvas(QRCanvas.getCanvas({
            width: size
          }), _objectSpread({
            cellSize: cellSize,
            data: foreground
      ...
      value: function makeLogo() {
        ...
        var _this$qrdata3 = this.qrdata,
            count = _this$qrdata3.count,
            cellSize = _this$qrdata3.cellSize,
            size = _this$qrdata3.size;
        ...
        var normalize = function normalize(k) {
          var margin2 = 2 * logo.margin;
          var height = size;
          var width = k * height;
          var iHeight = Math.min(count * Math.sqrt(logo.size / k), count / k) | 0;
          var iWidth = k * iHeight | 0; // (count - [iWidth | iHeight]) must be even if the logo is in the middle
        ...
        if (logo.image) {
          ...
          var tHeight = 100;
          var font = [logo.fontStyle, "".concat(tHeight, "px"), logo.fontFamily].filter(Boolean).join(' ');

          var _QRCanvas$measureText = QRCanvas.measureText(logo.text, font),
              tWidth = _QRCanvas$measureText.width;

          normalize(tWidth / tHeight);

          var _ctx = canvas.getContext('2d');

          _ctx.font = [logo.fontStyle, "".concat(logo.height, "px"), logo.fontFamily].filter(Boolean).join(' ');
      ...
      key: "make",
      value: function make() {
        var _this$qrdata4 = this.qrdata,
            cellSize = _this$qrdata4.cellSize,
            size = _this$qrdata4.size;
        var _this$cache = this.cache,
            background = _this$cache.background,
            foreground = _this$cache.foreground,
            logo = _this$cache.logo;
      ...
      value: function output() {
        ...
        var outputCanvas = QRCanvas.getCanvas({
          canvas: canvas,
          width: computedSize
        });
  ...
  function getCanvas(_ref) {
    var width = _ref.width,
        height = _ref.height,
        canvas = _ref.canvas;
    var rCanvas = canvas || cache.pop() || QRCanvas.createCanvas();
    rCanvas.width = width;
  ...
  function cacheCanvas(canvas) {
    if (Array.isArray(canvas)) cache.push.apply(cache, _toConsumableArray(canvas));else if (QRCanvas.isCanvas(canvas)) cache.push(canvas);
  }
  /**
  ...
  function measureText(text, font) {
    if (!canvasText) canvasText = getCanvas({
      width: 100
    });
  ...
  */
  ...
  function getEdger(_ref2) {
    ...
    if (level && level < 3) {
      ...
      } catch (e) {// The canvas has been tainted by cross-origin data.
      }
  ...
  var plugin = function plugin(qrcanvas) {
    ...
    events.on('clearLogo', function (_ref4) {
      ...
      if ((logo.image || logo.text) && logo.clearEdges === 1) {
        var canvasLogo = QRCanvas.getCanvas({
          width: logo.width + 2 * logo.margin,
          height: logo.height + 2 * logo.margin
        });
  ...
  function qrcanvas(options) {
    var canvas = new QRCanvas(options);
    canvas.makeAll();
    return canvas.output(options);
  }
  qrcanvas.QRCanvas = QRCanvas;

  var QRCanvas$1 = qrcanvas.QRCanvas;

  QRCanvas$1.createCanvas = function () {
    return document.createElement('canvas');
  };

  QRCanvas$1.isCanvas = function (el) {
    return el instanceof HTMLCanvasElement;
  };

  QRCanvas$1.isDrawable = function (el) {
    return QRCanvas$1.isCanvas(el) || el instanceof HTMLImageElement;
  };
